def discretizador(nombre):
    ...
    instancia = [ list(map(float,x[:len(x)-3])) for x in lectura ]
    instancia2 = [ list(map(float,x[:len(x)])) for x in lectura ]
    ...
    matriz = []
    intervalo = []
    for index_atributo in range(len(instancia[0])):
        auxiliar = []
        for index_registro in range(len(instancia)-1):
            auxiliar.append(instancia[index_registro][index_atributo])
        v_max = max(auxiliar)
        v_min = min(auxiliar)
    ...
    for j in range(len(instancia[0])):
        ...
        for i in range(len(instancia)):
            cont=1;
            if(instancia[i][j]<intervalos[j][0]):
                var = str(cont)
            else:
                cont = cont + 1
                for k in range(1,len(intervalos[j])-1,2):
                    if(instancia[i][j]>=intervalos[j][k] and instancia[i][j]<intervalos[j][k+1]):
                        var=str(cont)
                    cont = cont +1
                if (instancia[i][j] >= intervalos[j][len(intervalos[j])-1]):
                    var = str(cont)
            cadenas.append(var)
    ...
    for i in range(len(auxsalida)):
        ...
        linea = linea + str(instancia2[i][2]) + "," + str(instancia2[i][3]) + "," + str(instancia2[i][4])+"\n"
        salida.write(linea)
    salida.close()